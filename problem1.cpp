...
int main() {//課題１　f(x) = x^2 + x + 1
  ...
  string filename = "report-4-1-1211201118.txt";
	ofstream writing_file;
	writing_file.open (filename, ios::out);
	writing_file << "1211201118 林優花" << endl;
	writing_file << "課題1 f(x) = x^2 + x + 1 に最小値を与えるxを求める。" << endl;
	
	//勾配をもとめる。
  for ( i = 0; i < 500; i++ ) {//ステップ番号　0から499まで	
    //f(x)
    fx_vector[i] = x_vector[i] * x_vector[i] + x_vector[i] + 1.000000;
    //勾配を計算 esp 勾配は微分して絶対値　x を代入 
	  esp_vector[i] = 2.000000 * x_vector[i] + 1.000000;
